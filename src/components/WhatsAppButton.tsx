const DEFAULT_NUMBER = "919487789546";

export function WhatsAppButton() {
  const number = (process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || DEFAULT_NUMBER).replace(/\D/g, "");
  if (!number) return null;

  const message = encodeURIComponent("Hi! I have a question about your rudraksha.");
  ...
  return (
    <a
      ...
      className="group fixed bottom-5 right-5 z-40 flex items-center gap-2 rounded-full bg-[#25D366] px-4 py-3 text-white shadow-lg transition hover:bg-[#1ebe5b] hover:shadow-xl"
    >
